...
OUTPUT_PATH = sys.path[0] + "/static/outputs"
GIF_PATH = sys.path[0] + "/static/gif"
...
def setup():
    ...
    utils.safe_mkdir(OUTPUT_PATH)
    utils.safe_mkdir(GIF_PATH)


class StyleTransfer(object):
    def __init__(self, content_img="static/contents/thy.jpg",
                 style_img="static/styles/scream.jpg",
        ...
        # 当前混合图片路径
        self.mix_img = content_img

        # 规范化图片的像素尺寸
        content_img = sys.path[0] + '/' + content_img
        style_img = sys.path[0] + '/' + style_img
        ...
        utils.safe_mkdir(OUTPUT_PATH + "/%s_%s" % (self.content_name, self.style_name))
    ...
    def gif(self):
        file_name = self.content_name + '_' + self.style_name
        path = OUTPUT_PATH + '/' + file_name
        temp = utils.create_gif(path)
        self.res = temp.replace(sys.path[0], '')
        print("The gif has been produced!")


if __name__ == "__main__":
    setup()
    # 指定图片
    content = "static/contents/thy.jpg"
    style = "static/styles/scream.jpg"
    # 指定像素尺寸
    width = 400
    height = 300
    # style transfer
    style_transfer = StyleTransfer(content, style, width, height)
